...
class DQMTests(unittest.TestCase):
    ...
    * test_run_new_points – test the quality/coherence of built frames for new points
    ...
    def test_default_mass_for_num_dims(self):
        ...
        try:
            mass = dqm.default_mass_for_num_dims()
            success = False
        except AssertionError:
            success = True
        self.assertTrue(success, 'must raise AssertionError when no number of dimensions is available')

        # test that method does not raise an error when number of dimensions is available from dqm:-frames
        dqm.frames = np.zeros((10, 6, 5))
        try:
            mass = dqm.default_mass_for_num_dims()
            success = True
        except AssertionError:
        ...
        self.assertTrue(success,
                        'must not raise AssertionError when number of dimensions is available from dqm.frames')

        # test that suggested mass for 10 dimensions is 1
        mass = dqm.default_mass_for_num_dims(10)
        ...
        mass = dqm.default_mass_for_num_dims(100)
    ...
    def test_run_pca(self):
        ...
        dqm.raw_data = rng.random((100, 15))

        # test that all relevant instance member variables are set
        dqm.run_pca()
        not_none = [x is not None for x in [dqm.raw_col_means, dqm.pca_eigvecs, dqm.pca_eigvals, dqm.pca_cum_var]]
        ...
        success = np.isclose(dqm.pca_cum_var[-1], 1)
    ...
    def test_clear_pca(self):
        ...
        dqm.raw_data = rng.random((123, 21))
        dqm.run_pca()
        dqm.clear_pca()

        # test that all relevant instance member variables are reset
        is_none = [x is None for x in
                   [dqm.pca_num_dims, dqm.raw_col_means, dqm.pca_eigvecs, dqm.pca_eigvals, dqm.pca_cum_var]
                   ]
    ...
    def test_create_frame_0(self):
        ...
        dqm.raw_data = mat
        dqm.frames = None
        dqm.create_frame_0()  # note: *not* passing in raw data here
        ...
        frame0 = dqm.create_frame_0(mat)
        success = np.array_equal(frame0, dqm.frames)
        self.assertTrue(success,
                        'passing raw data to method must produce same result as accessing it from dqm.raw_data')
    ...
    def test_clear_basis(self):
        ...
        # test that all relevant instance member variables are reset
        success = dqm.basis_row_nums is None and dqm.non_basis_row_nums is None and dqm.basis_rows is None
        self.assertTrue(success, 'method must reset all relevant member variables')
    ...
    def test_build_operators(self):
        ...
        dqm.sigma = dqm.mean_row_distance
        dqm.build_operators()
        success = np.array_equal(dqm.simt.shape, [num_rows, num_rows]) and \
                  np.array_equal(dqm.xops.shape, [num_rows, num_rows, num_cols]) and \
                  np.array_equal(dqm.exph.shape, [num_rows, num_rows])
        ...
        dqm.sigma = dqm.mean_row_distance * 10
        dqm.build_operators()
        simt = np.copy(dqm.simt)
        xops = np.copy(dqm.xops)
        exph = np.copy(dqm.exph)
    ...
    def test_choose_basis_by_distance(self):
        ...
        dqm.raw_data = rng.random((163, 21))
        dqm.create_frame_0()

        # test with single chunk
        dqm.basis_start_with_outlier = True
        num_rows = dqm.raw_data.shape[0]
        test_basis_size = 37
        dqm.basis_size = test_basis_size
        dqm.choose_basis_by_distance()
        success = len(dqm.basis_row_nums) == test_basis_size and dqm.basis_rows.shape[0] == test_basis_size and \
                    len(dqm.non_basis_row_nums) == num_rows - test_basis_size
        self.assertTrue(success, 'basis must have the desired number of rows')

        # test with multiple chunks
        basis_row_nums1 = copy(dqm.basis_row_nums)
        dqm.basis_num_chunks = 2
        dqm.choose_basis_by_distance()
        success = dqm.basis_row_nums != basis_row_nums1
        self.assertTrue(success, 'basis rows must be different when running with multiple chunks')

        # test with start_with_outlier off
        dqm.basis_num_chunks = 1
        dqm.basis_start_with_outlier = False
        dqm.choose_basis_by_distance()
        success = dqm.basis_row_nums != basis_row_nums1
    ...
    def test_build_overlaps(self):
        ...
        row_nums = dqm.non_basis_row_nums
        sigma = 0.6
        batch_size = 100
        overlaps = dqm.build_overlaps(row_nums=row_nums, sigma=sigma, batch_size=batch_size)
        success = overlaps.size == num_rows - dqm.basis_size and \
        ...
        overlaps2 = dqm.build_overlaps(sigma=sigma, batch_size=batch_size)
        ...
        new_frame_0 = dqm.create_frame_0(new_rows)
        overlaps = dqm.build_overlaps(new_frame_0)
    ...
    def test_estimate_mean_row_distance(self):
        ...
        dqm.estimate_mean_row_distance()
        mu1 = dqm.mean_row_distance

        # test that multiplying raw data by 100 produces the right mean
        test_factor = 100
        dqm.frames = test_factor * mat[:, :, np.newaxis]
        dqm.estimate_mean_row_distance()
        mu2 = dqm.mean_row_distance
    ...
    def test_choose_sigma_for_basis(self):
        ...
        sigma = dqm.sigma
        test_ratio = sigma / dqm.mean_row_distance
        ...
        dqm.choose_sigma_for_basis(batch_size=batch_size, num_batches_to_test=num_batches_to_test)
        success = dqm.sigma == sigma
    ...
    def test_set_stopping_threshold(self):
        ...
        success = dqm.stopping_threshold is None
        ...
        dqm.set_stopping_threshold()
        success = dqm.stopping_threshold > 0 and dqm.stopping_threshold < dqm.mean_row_distance
    ...
    def test_build_frames(self):
        ...
        dqm.build_frames(num_frames_to_build)
        frames1 = np.copy(dqm.frames)
        success = dqm.frames.shape[2] == 240
        ...
        dqm.clear_frames()
        dqm.build_frames(num_frames_to_build, pare_frames=False)
    ...
    def test_build_frames_auto(self):
        ...
        dqm.build_frames(400)
        frames1 = np.copy(dqm.frames)
        dqm.clear_frames()
        batch_size = 5
        dqm.build_frames_auto(batch_size)
        frames2 = np.copy(dqm.frames)
        success = np.array_equal(frames1, frames2)
    ...
    def test_clear_frames(self):
        ...
        dqm.frames = np.zeros((10, 5, 3))  # create an all-zero array
        dqm.frames[0, 0, 0] = 1  # add a single 1 in the first frame
        dqm.clear_frames()
        success = dqm.frames.ndim == 3 and dqm.frames.shape[2] == 1 and dqm.frames[0, 0, 0] == 1
    ...
    def test_run_new_points(self):
        ...
        num_cols = 14
        dqm.raw_data = rng.random((num_rows, num_cols))
        dqm.pca_var_threshold = 0.8
        dqm.create_frame_0()

        dqm.basis_size = 75
        dqm.choose_basis_by_distance()

        dqm.overlap_mean_threshold = 0.8
        dqm.choose_sigma_for_basis()
    ...
    def test_run_simple(self):
        ...
        dqm2.sigma = sigma
        dqm2.create_frame_0()
        dqm2.build_operators()
        dqm2.build_frames_auto()

        # test that the 2 runs produce identical frames
        self.assertTrue(np.array_equal(dqm1.frames, dqm2.frames), 'frames must be identical')
    ...
    def test_python_code(self):
        ...
        basis_size = 10
        dqm1.basis_size = basis_size
        dqm1.choose_basis_by_distance()
        dqm2.basis_size = basis_size
        dqm2.choose_basis_by_distance()
        success = dqm1.basis_row_nums == dqm2.basis_row_nums
        ...
        dqm1.choose_sigma_for_basis()
        dqm2.choose_sigma_for_basis()
        success = dqm1.sigma == dqm2.sigma
        ...
        overlaps1 = dqm1.build_overlaps()
        overlaps2 = dqm2.build_overlaps()
        ...
        dqm1.build_operators()
        dqm1.build_frames_auto()
        dqm2.build_operators()
        dqm2.build_frames_auto()
        success = np.allclose(dqm1.frames, dqm2.frames)
